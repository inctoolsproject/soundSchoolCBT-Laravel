...
/* ======Amir Code======= */
    ...
    //hamza
    public function voucherTrack()
    {
        if (!$this->rbac->hasPrivilege('collect_fees', 'can_view')) {
            access_denied();
        }
        
        $data['title'] = 'Track Voucher';
        $data['searchlist'] = $this->customlib->get_searchtype();
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/voucher/voucher_list', $data);
            $this->load->view('layout/footer', $data);
            ...
            if (isset($id)) {
                $voucher = $this->Feevoucher_model->get($id);
                if (isset($voucher)) {
                    $this->load->view('invoice/invoice', ['data' => unserialize($voucher['content']), 'voucher_id' => $voucher['voucher_id'], 'billing_Month' => $voucher['feemonth']]);
                } else {
                ...
                $resultList         = $this->Feevoucher_model->search($date_from, $date_to);
                print_r("dhsfdhsg");
                die;
...
public function collection_report()
{
    ...
    if ($this->form_validation->run() == false) {
        ...
        $data['results'] = $this->studentfeemaster_model->getFeeCollectionReport($start_date, $end_date, $received_by, $group,$class_id,$section);
